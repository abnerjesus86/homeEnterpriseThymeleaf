/**
 * EIR - Codigo Javascript necesario para hacer funcionar el modulo EIR DI
 * Version 1.1 
 *
...
jQuery(function($) {
    ...
    var Option = '',
		...
		InitModalEvents = false,
		InitTableDamages = false,
		statusModalEvents = false,
        CntrConsecutivo,
        content = "",
        title = "",
        side = "",
		...
		op_SystemActive = true,
		op_Maintenance = false,
		op_MaintenanceTitle = '',
		op_MaintenanceText = '',
		op_Helps = false,
    ...
    jQuery.fn.extend({
		startSystem: function() {//Funcion que inicia el sistema
			if (opsInet) {
				var jsonDataObject = new Object();
				jsonDataObject.user = sUser;
				...
				$.ajax({
					...
					crossDomain : true,
					type: 'post', 
					url: 'http://10.130.6.1:7001/wseir/wsOCR/Settings', 
					dataType: 'json',
					data: jsonData,
					success: function (data)
					{	
						$.each(data, function(index, n1)
						{						
							if(n1.option === "WebServicesURLS" )
							{
								$.each(n1.content, function(index, n2)
								{
									if(n2.option == "op_WSContainers")
										op_WSContainers = n2.value;
									else if (n2.option == "op_WSEvents")
										op_WSEvents = n2.value;
									else if (n2.option == "op_WSDamages")
										op_WSDamages = n2.value;
									else if (n2.option == "op_WSSettingsUpdate")
										op_WSSettingsUpdate = n2.value;		
								}); 
							}
							else if(n1.option === "SystemOptions" )
							{
								$.each(n1.content, function(index, n2)
								{
									if(n2.option === "op_Maintenance")
										op_Maintenance = (n2.value === "1")?true:false;
									else if(n2.option === "op_MaintenanceTitle")
										op_MaintenanceTitle = n2.value;
									else if(n2.option === "op_MaintenanceText")
										op_MaintenanceText = n2.value;
									else if (n2.option === "op_SearchStartDate")
										op_SearchStartDate = n2.value;
									else if (n2.option === "op_SearchEndDate")
										op_SearchEndDate = n2.value;
								});
							}
							else if(n1.option === "MainOptions" )
							{
								$.each(n1.content, function(index, n2)
								{
									if(n2.option === "op_Helps")
										op_Helps = (n2.value === "1")?true:false;
									else if (n2.option === "op_FiltersSearch")
									{	
										op_FiltersSearch = (n2.value === "1")?true:false;

										if (op_FiltersSearch)
											$.each(n2.content, function(index, n3)
											{	
												if(n3.option === "op_FilterColvis")
													op_FilterColvis = (n3.value === "1")?true:false;									
												else if(n3.option === "op_FilterCopy")
													op_FilterCopy = (n3.value === "1")?true:false;
												else if(n3.option === "op_FilterCSV")
													op_FilterCSV = (n3.value === "1")?true:false;
												else if(n3.option === "op_FilterExcel")
													op_FilterExcel = (n3.value === "1")?true:false;
												else if(n3.option === "op_FilterPDF")
													op_FilterPDF = (n3.value === "1")?true:false;
												else if(n3.option === "op_FilterPrint")
													op_FilterPrint = (n3.value === "1")?true:false;											
											});
									}
								});
							}
							else if(n1.option === "GalleryOptions" )
							{
								$.each(n1.content, function(index, n2)
								{
									if(n2.option === "op_Zoom")
										op_Zoom = (n2.value === "1")?true:false;										
									else if(n2.option === "op_GalleryImages")
										op_GalleryImages = (n2.value === "1")?true:false;
									else if(n2.option === "op_GalleryVideos")
										op_GalleryVideos = (n2.value === "1")?true:false;
								});
							}			
						});
						...
						if(op_Maintenance)	
						{
							swal({  title: op_MaintenanceTitle,   
							text: op_MaintenanceText,
							html: true, 
							imageUrl: "../assets/images/Icon_SystemMaintenance.svg" });						
						}
						else
							$(this).initialize();							
					},
					error: function(e)
					{
						...
						console.log(e);
						swal("Oops...", "Ha ocurrido un error al cargar la configuración del usuario, intentelo de nuevo mas tarde por favor","error");
					}
				});
			}
			return false;
        ...
        initialize: function() {//Funcion: Inicializa alguno de los componentes		
			...
			$('SPAN.user-info').html('<small>Bienvenido,</small>'+sUser);
			...
			$('.input-daterange').datepicker({
				format: "dd/mm/yyyy",
				weekStart: 0,
				//startDate: $(this).getDate(op_SearchStartDate),
				//endDate: $(this).getDate(op_SearchEndDate),
				todayBtn: true,
				clearBtn: true,
				language: "es",
				autoclose: false,
        ...
        getSearchResults: function(p_Data) {//Funcion: Obtenemos los resultados de la busqueda
			...
			if (InitTableSearch) {
				$('#TablaPrincipal').cleanTable('Resultados');
			} else
			...
			$.ajax({
				...
				success: function(result) {
						...
						.DataTable({
							...
							columns: [{
								...
								render: function(data, type, row) {
									return "<div class='action-buttons center'><span class='selectedlink'></span><a href='#modal-events' role='button' data-toggle='modal'><b>" + data.Container + "</b></a></div>";
								},
								...
								data: null,
								render: function(data, type, row) {
									return (data.Damage === 0?"<span class='label label-sm label-primary arrowed arrowed-right'>NO</span>":"<div class='action-buttons center'><a href='#modal-damages' role='button' data-toggle='modal'><span class='label label-sm label-danger arrowed arrowed-right'>SI</span></a></div>");
								},
								sortable: false,
								className: "modal-damages"
							}, 							
							{
								title: "Evento",
								data: null,
								defaultContent: "<div class='action-buttons center'><a class='blue' href='#modal-events' role='button' data-toggle='modal' ><i class='ace-icon fa fa-search-plus bigger-150'></i></a></div>",								sortable: false,
								className: "center modal-events"
							}],
							...
							initComplete: function(oSettings, json) {
								...
								if (oSettings.fnRecordsTotal() > 0) {
									...
									$('#TablaPrincipal tbody .modal-damages a').click(function() {
										var ColumnaActual = $(this).parent().parent().parent().get(0),
										FilaActual = $('#TablaPrincipal').DataTable().row(ColumnaActual).data();
											
											$(this).getDamages(FilaActual);
									});
									
									$('#TablaPrincipal tbody .modal-events a').click(function() {										
											var ColumnaActual = $(this).parent().parent().parent().get(0),
												FilaActual = $('#TablaPrincipal').DataTable().row(ColumnaActual).data();
					...
					$('#TablaPrincipal').set_FiltersSearch(op_FiltersSearch);
				},
				error: function(e) {
					alert('error!: ' + e);
				}
        ...
        getEvents: function(p_Data) {//Funcion: Se obtienen los eventos para el consecutivo del contenedor
                ...
                $("#modal-events .modal-header h4").text("Eventos para el contenedor "+p_Data.Container);
                $('#loader-search-events').setCenter('50%').removeClass('hidden').show("fast");
							
				$('#modal-events').on('show.bs.modal', function () {
                ...
                $.ajax({
					...
					var	tableEvents = $('#TablaEventos').dataTable({
                            ...
                            initComplete: function(oSettings, json) {
                               ...
                               $('#TablaEventos tbody td.details-control').click(function() {
								...
								$('#TablaEventos tbody tr td #sp-simpleportfolio div div ul li a').click(function() {
									console.log('presinadooohhh!!!');
								});
					...
					error: function(e)
					{
						console.log('Ocurrio un error al cargar la tabla de eventos');
						console.log(e);
						swal("Oops...", "Ha ocurrido un error al cargar los eventos para este contenedor, intentelo de nuevo mas tarde por favor","error");
					}					
                });
            },
        getDamages: function (p_Data) {//Funcion: Devuelve la tabla de daños para cada contenedor
				
			$("#modal-damages .modal-header h4").text("Daños registrados para el contenedor "+p_Data.Container);
			 //$('#loader-search-damages').setCenter('50%').removeClass('hidden').show("fast");
						
			if (!statabiert) {
				statabiert = true;
			if (InitTableDamages)
                $('#TablaDamages').cleanTable('Daños');
			else
				InitTableDamages = true;
									
			var jsonDataObject = new Object();
			jsonDataObject.consecutive = 4220077; //p_Data.CntrConsecutivo;  //p_Data.CntrConsecutivo; //4220077
            var jsonData = JSON.stringify(jsonDataObject);
            jQuery.support.cors = true;
			...
			$.ajax({
                    ...
                    success: function(result) {
						...
						var tableDamages = $('#TablaDamages').dataTable({
                            aaData: result,
                            columns: [{
                                title: "Numero",
                                data: "intcNumber",
                            ...
                            initComplete: function(oSettings, json) {
                                //console.log('Tabla Daños Cargada Correctamente');
                                $('#loader-search-damages').addClass('hidden');
                            },
                            select: false,
                            bAutoWidth: false,                           
                            bPaginate: false,
        ...
        cleanTable: function(p_Table) { //Funcion: Reinicializa las tablas DataTable
            ...
            if (p_Table == 'Resultados') {
                ...
                $('#TablaEventos').hide();
            } else if (p_Table = 'Daños') {
                $('#TablaDamages').DataTable().destroy();
                $('#TablaDamages').hide();
			}
            return false;
        },
					...
					'<div class="row">' +
						'<div class="widget-toolbar pull-left">' +
							'<label>' +
								...
								'<input id="filtroImagenes" type="checkbox" class="ace ace-switch ace-switch-3" '+((op_GalleryImages)?'checked':'')+' value="1"/>' +
								'<span class="lbl middle" data-lbl="&nbsp;SI&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;NO"></span>'+
							'</label>' +							
							'&nbsp;&nbsp;' +
							'<label>' +
								'<span class="blue"><i class="ace-icon fa fa-video-camera bigger-150"></i></span>&nbsp;' +
								'<input id="filtroVideos" type="checkbox" class="ace ace-switch ace-switch-3" '+((op_GalleryVideos)?'checked':'')+' '+((d.tipoocr == 'CRANE')?'disabled':'')+' value="2"/>' +
								'<span class="lbl middle" data-lbl="&nbsp;SI&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;NO"></span>'+
							'</label>' +
						'</div>' +
		...
		getSettings: function(){ //Funcion: Inicializamos los valores de la configuracion.
			...
			$('#txt_WSEvents').val(op_WSEvents);
			 ...
			 $("button#btnSaveConfiguration").click(function(){
			 
				var jsonDataObject = new Object();
				
				jsonDataObject.User 			= 	sUser;
				jsonDataObject.Helps 			= 	$('#cb_Helps').prop("checked");
				jsonDataObject.FiltersSearch 	= 	$('#cb_FiltersSearch').prop("checked");
				jsonDataObject.FilterColvis 	= 	$('#cb_FilterColvis').prop("checked");
				jsonDataObject.FilterCopy 		= 	$('#cb_FilterCopy').prop("checked");
				jsonDataObject.FilterCSV 		= 	$('#cb_FilterCSV').prop("checked");
				jsonDataObject.FilterExcel 		= 	$('#cb_FilterExcel').prop("checked");
				jsonDataObject.FilterPDF 		= 	$('#cb_FilterPDF').prop("checked");
				jsonDataObject.FilterPrint 		= 	$('#cb_FilterPrint').prop("checked");
				jsonDataObject.Zoom 			= 	$('#cb_Zoom').prop("checked");
				jsonDataObject.GalleryImages 	= 	$('#cb_GalleryImages').prop("checked");
				jsonDataObject.GalleryVideos 	= 	$('#cb_GalleryVideos').prop("checked");
				
				var jsonSettings = JSON.stringify(jsonDataObject);
				jQuery.support.cors = true;			
				
				$.ajax({
					url: op_WSSettingsUpdate,
					data: jsonSettings,
					dataType: "json",
					type: "post",
					contentType: "application/json; charset=utf-8",
					success: function(result) {
						...
						swal({   
								...
								text: "¡Tus cambios fueron salvados correctamente!",   
								type: "success",   
