  ...
  (function($) {
    return $.fn.imgPreload = function(options) {
      ...
      settings = {
        ...
        spinner_src: '../assets/images/loaders/ring.svg'
      };
      if (options) {
	  ...
	  this.each(function(index,element) {
        var $image, $placeholder, $spinner_img, offset_left, offset_top,posimg;
        $image = $(this);
        ...
        $spinner_img.load(function() {
          $(this).css({
			left: (offset_left + $placeholder.width() / 2) + $(this).width()
          });
          return $(this).css({
			top: (offset_top + $placeholder.height() / 2) + $(this).height()
          });
        });
...
$('.sp-simpleportfolio-item img').imgPreload();	//